...
# all caclulates scores for food/time
allCalcCosts2 = [[],[],[],[],[],[],[]]
...
# --------------------------------------------------------------------- A Star with the A* with the Euclidean distance

# s: snake object
# i : iterator to keep track of where food will be
# slow: True go slow False go fast
def aStar_search_euclidean(s, i, slow): 
    from util import Queue
    global width, rows, snack, tempFood, startState, food
    ...
    def euclideanDistance(position):
        # uses distance as a score for heuristic
        xy1 = position
        xy2 = tempFood.pos
        return math.sqrt(abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1]))
    ...
    while 1:
        ...
        if current not in visited:
            ...
            if s.isGoalState(current):
                ...
                performActions(directions, slow)
                # print("A_Star number of actions:", len(directions))
                actionsList[5].append(len(directions))
                # print("A_Star score:", len(s.body))
                scoreList[5] = len(s.body)
                # scoreList[2] = s.score
            for childNode, direction, cost in s.getSuccessors(current):
                if childNode not in aStar_priorityqueue.heap:
                    if childNode in visited:  # make sure child is not in visited so we don't go backwards
                        continue
                    hCost = costs + cost + euclideanDistance(childNode)
                    aStar_priorityqueue.push((childNode, directions + [direction], costs + cost), hCost)


# --------------------------------------------------------------------- End of A Star 

# --------------------------------------------------------------------- A Star with the A* with the square Euclidean distance

# s: snake object
# i : iterator to keep track of where food will be
# slow: True go slow False go fast
def aStar_search_squared(s, i, slow):
    from util import Queue
    global width, rows, snack, tempFood, startState, food
    ...
    def squaredEuclideanDistance(position):
        # uses distance as a score for heuristic
        xy1 = position
        xy2 = tempFood.pos
        return (abs(xy1[0] * xy2[0]) + abs(xy1[1] * xy2[1]))
    ...
    while 1:
        ...
        if current not in visited:
            ...
            if s.isGoalState(current):
                ...
                performActions(directions, slow)
                # print("A_Star number of actions:", len(directions))
                actionsList[6].append(len(directions))
                # print("A_Star score:", len(s.body))
                scoreList[6] = len(s.body)
                # scoreList[2] = s.score
            for childNode, direction, cost in s.getSuccessors(current):
                if childNode not in aStar_priorityqueue.heap:
                    if childNode in visited:  # make sure child is not in visited so we don't go backwards
                        continue
                    hCost = costs + cost + squaredEuclideanDistance(childNode)
                    aStar_priorityqueue.push((childNode, directions + [direction], costs + cost), hCost)
...
# --------------------------------------------------------------------- A Star with the new heuristic

# s: snake object
# i : iterator to keep track of where food will be
# slow: True go slow False go fast
def aStar_search_diagonal(s, i, slow):
    from util import Queue
    global width, rows, snack, tempFood, startState, food
    ...
    def diagnalHeuristic(position):
        # uses distance as a score for heuristic
        xy1 = position
        xy2 = tempFood.pos
        var_one = xy1[0] - xy2[0]
        var_two = xy1[1] - xy2[1]
        var_three = min(var_one, var_two)
        return abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1]) + var_three
        ...
        if current not in visited:
            ...
            for childNode, direction, cost in s.getSuccessors(current):
                if childNode not in aStar_priorityqueue.heap:
                    if childNode in visited:  # make sure child is not in visited so we don't go backwards
                        continue
                    hCost = costs + cost + diagnalHeuristic(childNode)
                    aStar_priorityqueue.push((childNode, directions + [direction], costs + cost), hCost)
...
def runSearch(run):
    ...
    mySnake.reset(START_POS)
    print("RUNNING DIAGONAL_ASTAR#", run)
    start = time.time()
    for i in range(0, len(FOOD_POS)):
        aStar_search_diagonal(mySnake, i, goSlow)
    end = time.time()
    diag_astar_time = end - start
    mySnake.reset(START_POS)
    ...
    UCS_time = end - start
    mySnake.reset(START_POS)
    print("RUNNING EuclideanDistance#", run)
    start = time.time()
    for i in range(0, len(FOOD_POS)):
        aStar_search_euclidean(mySnake, i, goSlow)
    end = time.time()
    astar_EuclideanDistance_time = end - start
    mySnake.reset(START_POS)
    print("RUNNING aStar_search_squared#", run)
    start = time.time()
    for i in range(0, len(FOOD_POS)):
        aStar_search_squared(mySnake, i, goSlow)
    end = time.time()
    astar_Squared_time = end - start
    mySnake.reset(START_POS)
    ...
    AStar_actions_diagonal  = sum(actionsList[4])
    AStar_actions_euclidean = sum(actionsList[5])
    AStar_actions_squared = sum(actionsList[6])
    ...
    print("Total A_Star actions taken:", AStar_actions_diagonal)
    print("Total aStar search euclidean:", AStar_actions_euclidean)
    print("Total aStar search euclidean squared:", AStar_actions_squared)

    print("RAW SCORES [ DFS, BFS, ASTAR, UCS, Diagonal, Euclidean, Euclidean_Squared]: ")
    print(scoreList)
    calcScores = [0, 0, 0, 0, 0, 0, 0]
    ...
    calcScores[4] = (scoreList[4] / AStar_actions_diagonal) * 100
    calcScores[5] = (scoreList[5] / AStar_actions_euclidean) * 100
    calcScores[6] = (scoreList[6] / AStar_actions_squared) * 100
    print("DFS score:", calcScores[0])
    print("BFS score:", calcScores[1])
    print("A_Star score:", calcScores[2])
    print("UCS score:", calcScores[3])
    print("A_Star_diagonal score:", calcScores[4])
    print("A_Star_euclidean score:", calcScores[5])
    print("A_Star_euclidean squared score:", calcScores[6])


    calcScores2 = [0, 0, 0, 0, 0, 0, 0]
    calcScores2[0] = (scoreList[0] / DFS_time) 
    calcScores2[1] = (scoreList[1] / BFS_time) 
    calcScores2[2] = (scoreList[2] / astar_time) 
    calcScores2[3] = (scoreList[3] / UCS_time) 
    calcScores2[4] = (scoreList[4] / diag_astar_time)
    calcScores2[5] = (scoreList[5] / astar_EuclideanDistance_time)
    calcScores2[6] = (scoreList[6] / astar_Squared_time)
    print("DFS score:", calcScores2[0])
    print("BFS score:", calcScores2[1])
    print("A_Star score:", calcScores2[2])
    print("UCS score:", calcScores2[3])
    print("A_Star_diagonal score:", calcScores2[4])
    print("A_Star_euclidean score:", calcScores2[5])
    print("A_Star_euclidean squared score:", calcScores2[6])
    ...
    allCalcCosts[5].append(calcScores[5])
    allCalcCosts[6].append(calcScores[6])



    allCalcCosts2[0].append(calcScores2[0])
    allCalcCosts2[1].append(calcScores2[1])
    allCalcCosts2[2].append(calcScores2[2])
    allCalcCosts2[3].append(calcScores2[3])
    allCalcCosts2[4].append(calcScores2[4])
    allCalcCosts2[5].append(calcScores2[5])
    allCalcCosts2[6].append(calcScores2[6])
    ...
    my_file.write('{0:10}  {1:14}\n\n'.format("ASTAR_DIAGONAL ACTIONS:", AStar_actions_diagonal))
    my_file.write('{0:10}  {1:14}\n\n'.format("ASTAR_EULIDEAN ACTIONS:", AStar_actions_euclidean))
    my_file.write('{0:10}  {1:14}\n\n'.format("ASTAR_SQUARED_EULIDEAN ACTIONS:", AStar_actions_squared))
    ...
    my_file.write('{0:10}  {1:14}\n\n'.format("RAW ASTAR DIAGONAL SCORE:", scoreList[4]))
    my_file.write('{0:10}  {1:14}\n\n'.format("RAW ASTAR EULIDEAN SCORE:", scoreList[5]))
    my_file.write('{0:10}  {1:14}\n\n'.format("RAW ASTAR SQUARED EULIDEAN SCORE:", scoreList[6]))
    
    
    my_file.write('{0:10}  {1:14}\n'.format("CALC BFS SCORE (ACTIONS):", calcScores[0]))
    my_file.write('{0:10}  {1:14}\n'.format("CALC DFS SCORE (ACTIONS):", calcScores[1]))
    my_file.write('{0:10}  {1:14}\n'.format("CALC ASTAR SCORE (ACTIONS):", calcScores[2]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC UCS SCORE (ACTIONS):", calcScores[3]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC ASTAR DIAGONAL SCORE (ACTIONS):", calcScores[4]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC ASTAR EULIDEAN SCORE (ACTIONS):", calcScores[5]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC ASTAR SQUARED EULIDEAN SCORE (ACTIONS):", calcScores[6]))


    my_file.write('{0:10}  {1:14}\n'.format("CALC BFS SCORE (TIME):", calcScores2[0]))
    my_file.write('{0:10}  {1:14}\n'.format("CALC DFS SCORE (TIME):", calcScores2[1]))
    my_file.write('{0:10}  {1:14}\n'.format("CALC ASTAR SCORE (TIME):", calcScores2[2]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC UCS SCORE (TIME):", calcScores2[3]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC ASTAR DIAGONAL SCORE (TIME):", calcScores2[4]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC ASTAR EULIDEAN SCORE (TIME):", calcScores2[5]))
    my_file.write('{0:10}  {1:14}\n\n'.format("CALC ASTAR SQUARED EULIDEAN SCORE (TIME):", calcScores2[6]))
    my_file.close()

    # -------------------------------------------------------------- Bar Graph for Scores
    data = {"Algorithm": ["DFS", "BFS", "ASTAR", "UCS", "ASTAR_DIAGONAL", "ASTAR_EULIDEAN", "ASTAR_SQUARED_EULIDEAN"],
    ...
    data = {"Algorithm": ["DFS", "BFS", "ASTAR", "UCS", "ASTAR_DIAGONAL", "ASTAR_EULIDEAN", "ASTAR_SQUARED_EULIDEAN"],
            ...
            "ScoreTwo": [round(x,2) for x in calcScores2]

            }
    # Dictionary loaded into a DataFrame
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    ax = dataFrame.plot(kind='bar',x="Algorithm", y="ScoreTwo", rot=70,
                       title="Scores of Snake Game Search Algorithms " + str(datetime.now()))
    ...
    print(len(actionsList[5]))
    print(len(actionsList[6]))

    maxLen = max(len(actionsList[0]),len(actionsList[1]),len(actionsList[2]),len(actionsList[3]), len(actionsList[4]), len(actionsList[5]), len(actionsList[6]))
    ...
    for x in range(0,maxLen):
        ...
        if len(actionsList[5]) < maxLen+1:
            actionsList[5].append(0)
        if len(actionsList[6]) < maxLen+1:
            actionsList[6].append(0)
    ...
    # -------------------------------------------------------------- ASTAR_DIAGONAL Line Graph for Actions

    data = {"Score": scoreIdx,

            "ASTAR_DIAGONAL": actionsList[4]

            }
    # Dictionary loaded into a DataFrame
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="Score", y="ASTAR_DIAGONAL", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))

    plot.show(line=True)

    # -------------------------------------------------------------- ASTAR_EULIDEAN Graph for Actions

    data = {"Score": scoreIdx,

            "ASTAR_EULIDEAN": actionsList[5]

            }
    # Dictionary loaded into a DataFrame
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="Score", y="ASTAR_EULIDEAN", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))

    plot.show(line=True)

    # -------------------------------------------------------------- ASTAR_SQUARED_EULIDEAN Graph for Actions

    data = {"Score": scoreIdx,

            "ASTAR_SQUARED_EULIDEAN": actionsList[5]

            }
    # Dictionary loaded into a DataFrame
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="Score", y="ASTAR_SQUARED_EULIDEAN", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))
...
# -------------------------------------------------------------- DFS Line Graph for Actions
    ...
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="ScoreTwo", y="DFS", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))
    ...
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="ScoreTwo", y="BFS", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))
    ...
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="ScoreTwo", y="ASTAR", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))
    ...
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="ScoreTwo", y="ASTAR_DIAGONAL", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))
    ...
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="ScoreTwo", y="ASTAR_EULIDEAN", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))
    ...
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="ScoreTwo", y="ASTAR_SQUARED_EULIDEAN", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " + str(datetime.now()))
    ...
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    dataFrame.plot(kind='line',x="ScoreTwo", y="UCS", rot=70,
                       title="Actions of Snake Game Search Algorithms " + str(run) + " " +str(datetime.now()))
...
def runMultiple(times):
    ...
    averageCalcCosts[4] = sum(allCalcCosts[4])/times
    averageCalcCosts[5] = sum(allCalcCosts[5])/times
    averageCalcCosts[6] = sum(allCalcCosts[6])/times


    averageCalcCosts2[0] = sum(allCalcCosts2[0])/times
    averageCalcCosts2[1] = sum(allCalcCosts2[1])/times
    averageCalcCosts2[2] = sum(allCalcCosts2[2])/times
    averageCalcCosts2[3] = sum(allCalcCosts2[3])/times
    averageCalcCosts2[4] = sum(allCalcCosts2[4])/times
    averageCalcCosts2[5] = sum(allCalcCosts2[5])/times
    averageCalcCosts2[6] = sum(allCalcCosts2[6])/times
    ...
    data = {"Algorithm": ["DFS", "BFS", "ASTAR", "UCS", "ASTAR_DIAGONAL", "ASTAR_EULIDEAN", "ASTAR_SQUARED_EULIDEAN" ],
            ...
            "AvgScore2": [round(x, 2) for x in averageCalcCosts2]

            }
    # Dictionary loaded into a DataFrame
    dataFrame = pd.DataFrame(data=data)
    # Draw a vertical bar chart
    ax = dataFrame.plot(kind='bar', x="Algorithm", y="AvgScore2", rot=70,
                            title="Scores of Snake Game Search Algorithms Avg " + str(datetime.now()))
